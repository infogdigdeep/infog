...
function loadTable() {
	$.get(urlHolder.records, function(response) {
 		...
 		for (var i=0; i<response.records.length; i++) {
			...
			row += '<td>' + response.records[i].username + '</td>';
			row += '<td>' + response.records[i].firstName + '</td>';
			row += '<td>' + response.records[i].lastName + '</td>';
			row += '<td>' + getRole(response.records[i].role.role) + '</td>';
			row += '</tr>';
	 		$('#tableUsers').append(row);
 		}
 		
 		$('#tableUsers').data('model', response.records);
		toggleForms('hide'); ;
 	});
}
